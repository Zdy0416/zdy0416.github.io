...
`Web/` 工作区只保留本目录这一份最新版网站。旧测试站、动画制作中间文件及过期验收资料已移出项目；网站使用的素材均保存在本目录内。
...
- `index.html` 与 `assets/home-spiral.js`：两屏首页，01/ 标题首页 → 02/ 螺旋作品展示。下滑按钮直接进入作品，Back to Home 返回首页。
- 个人照片、介绍屏、首页底部联系屏及空白页脚已移除。作品区 Connect 直接打开邮件。
- `assets/projects-webgl.js`：首页第二屏的 3D 渲染器。顶部及手机菜单的 Projects 在首页平滑滚到第二屏，从其他页面返回 `/#all-projects`。
...
在源码文件夹运行 `pnpm install`，然后 `pnpm dev`，或双击 `start-local.command`。本地预览统一使用 http://127.0.0.1:5174/。
...
`dist/` 是可重新生成的构建产物，不保留历史构建副本。`node_modules/` 保留当前运行依赖；Git 历史、锁文件和发布配置继续保留。临时截图与验收产物放在已忽略的 `qa/` 或 `tmp/` 中，完成后清理。
